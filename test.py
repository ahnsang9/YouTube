...
def bs_crawling(url,list):
    ...
    list[0].append(title)
    list[1].append(soup.select('#downloadfrm > div > div > div.entry > div.info > div.artist')[0].text)
    ...
    try:
        ...
        list[5].append('')  # 19금은 로그인해야 가사 나옴 (bs4로는 어렵)


def playlist_crawling (number_of_songs, list_name, download_path):
    ...
    for i in tqdm(range(number_of_songs // 50 + 1), desc='플레이리스트 정보 다운중...'):
        ...
        for j in range(len(song_url)):
            ...
            urlretrieve(img_src, download_path + '\%s' % list_name + '\%s.jpg' % (info[0][50 * i + j]))


def url_crawling (info):
    element = driver.find_element_by_name("search_query")  # search창 지정
    element.clear()  # search창 클리어
    element.send_keys(f'{info[0][i]} {info[1][i]} mp3 lyrics')
...
def youtube_down (url, info):
        ...
        my_stream.download(download_path + '\%s' % list_name, '%s' % info)
        #except:pass


def trans_mp3 (list_name):
    os.chdir(download_path + '\%s' % list_name)
    temp = glob.glob('*.mp4')
...
if __name__ == "__main__":
    ...
    createFolder(download_path + '\%s'%list_name)

    #pool = Pool(processes=4)
    #pool.map(, )
    playlist_crawling(number_of_songs,list_name,download_path)
    ...
    trans_mp3(list_name, download_path)

    for i in range(number_of_songs):  # 태그 초기화
        try :
            song = eyed3.load(download_path + "\%s\%s.mp3" %(list_name, info[0][i]))
            song.tag.title = '%s' % info[0][i]
            song.tag.artist = '%s' % info[1][i]
            song.tag.album = '%s' % info[2][i]
            song.tag.track_num = i
            song.tag.images.set(3, open(download_path + "\%s\%s.jpg" %(list_name, info[0][i]), 'rb').read(), 'image/jpeg')
            song.tag.lyrics.set(info[5][i])
            song.tag.save(version=eyed3.id3.ID3_V2_3)
        except:pass

    [os.remove(f) for f in glob.glob(download_path + "\%s"%list_name +'\*.jpg')]
    [os.remove(f) for f in glob.glob(download_path + "\%s"%list_name +'\*.mp4')]
