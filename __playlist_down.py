...
if __name__ == '__main__':
    ...
    options.add_argument('window-size=2560x1440')  # 브라우저 윈도우 사이즈
    ...
    df.transpose().to_excel(download_path + '\%s\info.xlsx'%list_names[num])
    driver.quit()
    yt_urls = []


    for i in tqdm(range(number_of_songs),desc='url 복사중..'):
        results = YoutubeSearch(f"\"{info[0][i]} {info[1][i]}\" +audio .video", max_results=1).to_dict()
        yt_urls.append('https://www.youtube.com'+results[0]['url_suffix'])
